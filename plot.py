...
def plot_all(grid, temperatures):
    nodes_x = []
    nodes_y = []
    elements = quads_to_tris(grid.elements)
    for i in range(len(grid.nodes)):
        nodes_x.append(grid.nodes[i].x)
        nodes_y.append(grid.nodes[i].y)
    triangulation = tri.Triangulation(nodes_x, nodes_y, elements)
    plot_fem_mesh(nodes_x, nodes_y, elements)
    plt.tricontourf(triangulation, temperatures)
    plt.colorbar()
    plt.axis('equal')
    plt.show()
